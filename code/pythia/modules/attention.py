...
class TopDownAttention(nn.Module):
    ...
    def __init__(self, combination_layer, transform_module, normalization):
        ...
        self.combination_layer_oo = combination_layer
        self.transform_oo = transform_module

        self.combination_layer_ot = combination_layer
        self.transform_ot = transform_module

        self.combination_layer_tt = combination_layer
        self.transform_tt = transform_module

        self.combination_layer_to = combination_layer
        self.transform_to = transform_module

        self.vis_dim = 2048  # the dim of input image's visual feature: v
        self.context_dim = 300
        self.jemb_dim = 2048  # the dim of all processed features are same
        self.context_jemb_dim = 300
    ...
    def forward(self, attr, image_feat, question_embedding, image_locs=None,
                oo_edge_feature=None, s_oo=None,
                ot_edge_feature=None, s_ot=None):
        """
        :param attr: 'image' or 'context'
        :param image_feat: torch.Size([batch_size, 100, 2048])
        :param question_embedding: torch.Size([batch_size, 2048])
        :param image_locs:
        :return:
        """
        if attr == 'image':
            # N x K x joint_dim
            joint_feature = self.combination_layer(image_feat, question_embedding, attr=attr)  # torch.Size([3, 100, 5000])
            # N x K x n_att
            raw_attn = self.transform(joint_feature)  # torch.Size([3, 100, 1])
            ...
            """MLP for oo_edge feats"""
            if (oo_edge_feature is not None) and (s_oo is not None):
                joint_feature = self.combination_layer_oo(oo_edge_feature, s_oo, attr=attr)
                # N x K x n_att
                raw_attn_oo = self.transform_oo(joint_feature)
                attention_oo = nn.functional.softmax(raw_attn_oo, dim=1)

                if image_locs is not None:
                    oo_masked_attention = self._mask_attentions(attention_oo, image_locs)
                    oo_masked_attention_sum = torch.sum(oo_masked_attention, dim=1, keepdim=True)
                    oo_masked_attention_sum += oo_masked_attention_sum.eq(0).float() + self.EPS
                    oo_masked_attention = oo_masked_attention / oo_masked_attention_sum
                else:
                    oo_masked_attention = attention_oo

                """MLP for ot_edge feats"""
                if (ot_edge_feature is not None) and (s_ot is not None):
                    joint_feature = self.combination_layer_ot(ot_edge_feature, s_ot,
                                                           attr=attr)
                    # N x K x n_att
                    raw_attn_ot = self.transform_ot(joint_feature)
                    attention_ot = nn.functional.softmax(raw_attn_ot, dim=1)

                    if image_locs is not None:
                        ot_masked_attention = self._mask_attentions(attention_ot, image_locs)
                        ot_masked_attention_sum = torch.sum(ot_masked_attention, dim=1, keepdim=True)
                        ot_masked_attention_sum += ot_masked_attention_sum.eq(0).float() + self.EPS
                        ot_masked_attention = ot_masked_attention / ot_masked_attention_sum
                    else:
                        ot_masked_attention = attention_ot

                    return masked_attention, oo_masked_attention, ot_masked_attention
                else:
                    return masked_attention, oo_masked_attention
            ...
            # N x K x joint_dim
            joint_feature = self.combination_layer(image_feat, question_embedding, attr=attr)
            # N x K x n_att
            ...
            """MLP for oo_edge feats"""
            if (oo_edge_feature is not None) and (s_oo is not None):
                joint_feature = self.combination_layer_tt(oo_edge_feature, s_oo, attr=attr)
                # N x K x n_att
                raw_attn_oo = self.transform_tt(joint_feature)
                attention_oo = nn.functional.softmax(raw_attn_oo, dim=1)
                ...
                """MLP for ot_edge feats"""
                if (ot_edge_feature is not None) and (s_ot is not None):
                    joint_feature = self.combination_layer_to(ot_edge_feature, s_ot,
                                                           attr=attr)
                    # N x K x n_att
                    raw_attn_ot = self.transform_to(joint_feature)
                    attention_ot = nn.functional.softmax(raw_attn_ot, dim=1)
