...
class ClassifierLayer(nn.Module):
    def __init__(self, classifier_type, in_dim, out_dim, **kwargs):
        ...
        elif classifier_type == "dot_product":
            self.module = DotProductClassifier(in_dim, out_dim, **kwargs)
...
class DotProductClassifier(nn.Module):
    def __init__(self, in_dim, out_dim, **kwargs):
        ...
        input_dim = 768*2
        num_ans_candidates = out_dim
        self.linear_logit = nn.Linear(input_dim, num_ans_candidates-50)
        self.query = nn.Linear(768*2, 768)
        self.key = nn.Linear(768, 768)

    def forward(self, joint_embedding, ocr_tokens, ocr_mask):
        extended_attention_mask = (1.0 - ocr_mask) * -10000.0
        assert extended_attention_mask.dim() == 2
        extended_attention_mask = extended_attention_mask.unsqueeze(1)

        text_val = self.linear_logit(joint_embedding)
        logit_value = text_val   
        logit_value = logit_value.squeeze()   
        query_layer = self.query(joint_embedding)
        key_layer = self.key(ocr_tokens)

        dot_product = torch.matmul(query_layer, key_layer.transpose(-1, -2))
        dot_product = dot_product / math.sqrt(768)
        dot_product = dot_product + extended_attention_mask
        dot_product = dot_product.squeeze()
        concat_product_value = torch.cat((logit_value, dot_product), -1)
        return concat_product_value
...
class NonLinearElementMultiply(nn.Module):
    def __init__(self, image_feat_dim, ques_emb_dim, **kwargs):
        ...
        self.fa_txt = ReLUWithWeightNormFC(1536, kwargs["hidden_dim"])
    ...
    def forward(self, image_feat, question_embedding, context_embedding=None, question_embedding222=None, attr=None):
        if attr == 'image':
            ...
            # image_fa -> edge_fa, [batch_size, node_num, node_num, 2048]
            image_fa = self.fa_image(image_feat)  # need to check whether need view!
            question_fa = self.fa_txt(question_embedding)
            ...
            if context_embedding is not None:
                ...
                if question_embedding222 is not None:
                    question_fa_ocr = self.fa_txt(question_embedding222)
                    if len(image_feat.size()) == 3:
                        question_fa_expandOCR = question_fa_ocr.unsqueeze(1)
                    else:
                        question_fa_expandOCR = question_fa_ocr
                    context_text_joint_feaure = context_fa * question_fa_expandOCR
                else:
                    context_text_joint_feaure = context_fa * question_fa_expand
                # joint_feature = torch.cat([joint_feature, context_text_joint_feaure], dim=1)
            ...
            return joint_feature, context_text_joint_feaure
