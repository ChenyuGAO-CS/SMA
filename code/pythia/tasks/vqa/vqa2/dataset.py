...
class VQA2Dataset(BaseDataset):
    ...
    def load_item(self, idx):
        ...
        current_sample.obj_mask = min(self.copy_processor(
            {"blob": sample_info["obj_normalized_boxes"]}
        )["mask"], 36)
    ...
    def add_ocr_details(self, sample_info, sample):
        ...
        if "ocr_info" in sample_info:
            sample.ocr_bbox = self.bbox_processor({"info": sample_info["ocr_info"], "feats": context["text"], "img_id": sample.image_id, "obj_bbox": sample.obj_bbox})[
                "bbox"
            ]
